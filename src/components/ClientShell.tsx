        ...
        if (cursor && cursorCore) {
          ...
          const onEnter = (event: PointerEvent) => {
            if ((event.target as HTMLElement | null)?.closest("a, button, [role='button']")) {
              gsap.to(cursorCore, { scale: 1.18, borderColor: "rgba(197,131,81,0.66)", backgroundColor: "rgba(197,131,81,0.18)", duration: 0.16, ease: "power2.out" });
            }
          };
          const onLeave = () => {
            gsap.to(cursorCore, { scale: 1, borderColor: "rgba(197,131,81,0.5)", backgroundColor: "rgba(197,131,81,0.1)", duration: 0.16, ease: "power2.out" });
          };
