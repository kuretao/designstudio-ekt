...
export default function ContactModal({ open, onClose, promoTitle }: Props) {
  ...
  return (
    <div
      ...
      style={{ background: "rgba(13,13,11,0.82)", backdropFilter: "blur(12px)" }}
    >
      <div
        className="relative w-full max-w-md overflow-hidden rounded-[2rem] border border-white/12 bg-[#111110]"
        style={{ boxShadow: "0 40px 120px rgba(0,0,0,0.7), 0 0 0 1px rgba(215,196,161,0.08)" }}
      >
        {/* Glow accent */}
        <div className="pointer-events-none absolute -top-24 left-1/2 h-48 w-64 -translate-x-1/2 rounded-full bg-[#d7c4a1]/10 blur-3xl" />

        {/* Header */}
        <div className="relative px-7 pb-4 pt-7">
          <button
            onClick={onClose}
            aria-label="Закрыть"
            className="absolute right-5 top-5 flex h-8 w-8 items-center justify-center rounded-full border border-white/10 text-white/40 transition hover:border-white/30 hover:text-white"
          >
          ...
          <h2 className="text-2xl font-light tracking-[-0.035em] text-[#f3efe7]">
            {promoTitle ? "Записаться на акцию" : "Выберите удобный способ"}
          </h2>
          {promoTitle && (
            <p className="mt-2 text-sm leading-relaxed text-[#d8d1c4]">
              «{promoTitle}»
            </p>
          )}
        </div>
        ...
        <div className="space-y-3 px-7 py-5">
          {messengers.map((m) => (
            <a
              ...
              className={`group flex items-center gap-4 rounded-[1.25rem] border bg-gradient-to-r px-5 py-4 transition duration-300 ${m.color} ${m.glow}`}
            >
              <span className={`transition duration-300 group-hover:scale-110 ${m.accent}`}>
                {m.icon}
              </span>
              <div className="flex-1">
                <p className={`text-sm font-medium ${m.accent}`}>{m.label}</p>
                <p className="text-xs text-white/45">{messengerLinks.phone}</p>
              </div>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-4 w-4 text-white/25 transition duration-300 group-hover:translate-x-0.5 group-hover:text-white/60">
        ...
        <div className="px-7 pb-4">
          <a
            ...
            className="flex items-center gap-4 rounded-[1.25rem] border border-white/8 bg-white/[0.03] px-5 py-4 transition duration-300 hover:border-white/20 hover:bg-white/[0.06]"
          >
            <span className="text-[#d7c4a1]"><PhoneIcon /></span>
            <div className="flex-1">
              <p className="text-sm text-white/80">Позвонить</p>
              <p className="text-xs text-white/40">{messengerLinks.phone}</p>
            </div>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-4 w-4 text-white/25 transition group-hover:text-white/60">
              <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" />
            </svg>
          </a>
        </div>
        ...
        {/* Footer note */}
        <p className="px-7 pb-7 text-center text-xs leading-relaxed text-white/25">
