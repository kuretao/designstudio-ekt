  ...
  return (
    <section className="page-in relative min-h-screen overflow-hidden px-5 pb-16 pt-28 md:px-10 lg:px-16">
      ...
      <div className="relative z-10 mx-auto grid min-h-[calc(100vh-7rem)] max-w-7xl gap-10 lg:grid-cols-[0.92fr_1.08fr] lg:items-end">
        <div className="pb-8">
          <p className="text-xs uppercase tracking-[0.45em] text-[#C58351]">Error / 404</p>
        ...
        <div className="mb-8 grid gap-4">
          <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#050505]/48 p-6 backdrop-blur">
            <p className="text-xs uppercase tracking-[0.34em] text-[#C58351]">Быстрый маршрут</p>
            <div className="mt-6 grid gap-3">
              {quickLinks.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="group flex items-center justify-between gap-5 border-b border-white/10 pb-4 transition duration-300 last:border-b-0 last:pb-0 hover:border-[#C58351]/55"
                >
                  <span>
                  ...
                  <span className="text-2xl text-[#C58351] transition duration-300 group-hover:translate-x-1">→</span>
                </Link>
              ))}
            </div>
          </div>
