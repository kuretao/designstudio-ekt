  ...
  return (
    <div className="page-in pt-24">
      <section className="relative min-h-screen overflow-hidden px-5 pb-20 pt-28 md:px-10 lg:px-16">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(13,13,11,.98),rgba(13,13,11,.70),rgba(13,13,11,.28)),radial-gradient(circle_at_72%_18%,rgba(215,196,161,.22),transparent_34%)]" />

        <div className="relative z-10 mx-auto grid min-h-[calc(100vh-8rem)] max-w-7xl gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-end">
          <div>
            <p className="mb-5 text-xs uppercase tracking-[0.45em] text-[#d7c4a1]">{item.eyebrow}</p>
            <h1 className="max-w-4xl text-5xl font-light leading-[0.94] tracking-[-0.06em] md:text-7xl lg:text-8xl">
              {item.title}
            </h1>
            <p className="mt-7 max-w-2xl text-lg leading-relaxed text-[#d8d1c4] md:text-xl">{item.text}</p>
          ...
          <GlassPanel className="rounded-[2rem] p-6 md:p-8">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-[1.25rem] border border-white/10 bg-black/20 p-5">
                ...
                <strong className="mt-3 block text-3xl font-light text-[#d7c4a1]">{item.price}</strong>
              </div>
              <div className="rounded-[1.25rem] border border-white/10 bg-black/20 p-5">
            ...
            <div className="mt-5 grid gap-3">
              {item.deliverables.map((entry) => (
                <div key={entry} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.035] px-4 py-3 text-sm text-[#d8d1c4]">
                  <span className="h-1.5 w-1.5 rounded-full bg-[#d7c4a1]" />
                  {entry}
                </div>
              ))}
            </div>
      ...
      <section className="border-t border-white/10 px-5 py-24 md:px-10 lg:px-16">
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[0.8fr_1.2fr]">
          <div>
            <p className="mb-5 text-xs uppercase tracking-[0.45em] text-[#d7c4a1]">Почему это работает</p>
      ...
      <section className="px-5 py-24 md:px-10 lg:px-16">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 flex flex-col justify-between gap-5 md:flex-row md:items-end">
            ...
            <p className="max-w-xl text-[#d8d1c4]">
          ...
          <div className="grid gap-px overflow-hidden rounded-[2rem] border border-white/10 bg-white/10 md:grid-cols-5">
            {item.process.map((step, index) => (
              <div key={step} className="bg-[#0d0d0b] p-6">
                <span className="mb-12 block text-sm text-[#d7c4a1]">0{index + 1}</span>
